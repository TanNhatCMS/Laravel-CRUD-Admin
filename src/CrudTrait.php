...
trait CrudTrait
{
    ...
    */
    ...
    public static function getPossibleEnumValues($field_name)
    {
        ...
        $type = DB::select(DB::raw('SHOW COLUMNS FROM `'.$instance->getConnectionName().'`.`'.Config::get('database.connections.'.env('DB_CONNECTION').'.prefix').$instance->getTable().'` WHERE Field = "'.$field_name.'"'))[0]->Type;
